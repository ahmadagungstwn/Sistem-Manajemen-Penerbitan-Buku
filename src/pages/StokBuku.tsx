...
export default function StokBuku() {
  ...
  const filteredStok = stok?.filter(item => {
    ...
    return bukuData?.judul.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getBukuJudul = (isbn: string) => buku?.find(b => b.isbn === isbn)?.judul || '-';
  const getRakKode = (id: string) => rak?.find(r => r.rak_id === id)?.kode_rak || '-';
    ...
    <DashboardLayout>
      <div className="p-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Data Stok Buku</h1>
            <p className="text-muted-foreground">Kelola stok buku di inventaris</p>
          </div>
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingId ? 'Edit Stok' : 'Tambah Stok Baru'}</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, stok_id: e.target.value })}
                    disabled={!!editingId}
                    placeholder="STK001"
                    required
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setFormData({ ...formData, rak_id: value })}
                  >
                    <SelectTrigger>
                  ...
                  {filteredStok && filteredStok.length > 0 ? (
                    filteredStok.map((item) => (
                      <TableRow key={item.stok_id}>
                        <TableCell className="font-medium">{item.stok_id}</TableCell>
                        ...
                        <TableCell className="text-right">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleDelete(item.stok_id)}>
